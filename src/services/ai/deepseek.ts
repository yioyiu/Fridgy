...
class DeepSeekService {
  ...
  async analyzeLocationStorageTime(itemName: string, location: string): Promise<LocationStorageAnalysisResult | null> {
    ...
    try {
      console.log('调用DeepSeek分析位置存储时间:', itemName, location);

      const currentLanguage = getCurrentLanguage();
      const prompt = `Please analyze the optimal storage duration for "${itemName}" in "${location}".
...
Consider factors:
1. Item type and characteristics
2. Environmental conditions of storage location
3. Food safety standards
4. Optimal consumption period

Please respond in ${currentLanguage === 'zh' ? 'Chinese' : 'English'}.`;
      ...
      console.log('DeepSeek位置分析返回结果:', content);
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          return {
            days: result.days || 7,
            reason: result.reason || 'Suggested storage duration'
          };
        }
      } catch (parseError) {
      ...
      // 如果JSON解析失败，尝试文本解析
      const daysMatch = content.match(/(\d+)\s*(?:天|days?)/i);
      ...
      console.error('DeepSeek Error analyzing location storage time:', error);
      return null;
    }
  }
