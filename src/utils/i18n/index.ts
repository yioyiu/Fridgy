...
const TRANSLATIONS = {
  en: {
    ...
    dashboard: {
      ...
      used: 'Used',
      total: 'Total',
      usedHint: 'Tap restore button to restore',
    },
    // Overview
    overview: {
    ...
    statistics: {
      ...
      freshnessScoreCalculation: 'How is the freshness score calculated?',
      ...
      freshnessFormula: 'Freshness Score = (Fresh × 1.0 + Near Expiry × 0.5 + Expired × 0.0 + Used × 0.8) ÷ Total Items × 100%',
      example: 'Example',
      freshnessExample: 'If you have 8 fresh items, 2 near expiry, 1 expired, and 4 used items (15 total), your freshness score would be: (8×1.0 + 2×0.5 + 1×0.0 + 4×0.8) ÷ 15 × 100% = 13.2 ÷ 15 × 100% = 88%',
      defaultScoreRule: 'Default Score Rule',
      defaultScoreExplanation: 'When there are no items in a week, the freshness score is 100% (perfect score), indicating excellent management with no waste risk.',
      weeklyFreshnessTrend: 'Weekly Freshness Trend',
      freshnessTrend: 'Freshness Trend',
    ...
    settings: {
      ...
      about: 'About',
      version: 'Version',
      versionStoreError: 'Unable to open the app store. Please manually search for "Fridgy" in your app store.',
      legal: 'Legal',
      ...
      versionUpdate: {
        ...
        errorMessage: 'Unable to open the app store for rating. Please manually go to the app store and search for "Fridgy" to rate.',
    ...
    support: {
      ...
      faqQ6: 'How are statistics calculated in the Statistics page?',
      faqA6: 'The key metrics and status overview in the Statistics page are calculated based on ingredients added within the selected time period. For example, when you select "Week", it shows statistics for ingredients added from Monday to today of the current week.',
      faqQ7: 'How do I use voice input to add ingredients?',
      faqA7: 'On the ingredient add page, tap and hold the microphone button to record your ingredient details. The app will use AI to analyze and fill in the information automatically.',
      faqQ8: 'What is the AI analysis feature?',
      faqA8: 'The AI analysis feature provides intelligent suggestions for ingredient storage locations and cooking advice based on your ingredients. It can also help with meal planning and waste reduction tips.',
      faqQ9: 'How do I set up notifications?',
      faqA9: 'Go to Settings > Notifications to enable daily reminders, near expiry alerts, and expired item alerts. You can also set custom reminder times.',
      faqQ10: 'How do I edit or delete ingredients?',
      faqA10: 'Tap on any ingredient card to edit it, or swipe left on the ingredient to reveal the delete option. You can also use the long-press menu for more options.',
      faqQ11: 'What are the different ingredient statuses?',
      faqA11: 'Fresh (green): Good to use. Near Expiry (orange): Use soon. Expired (red): Needs disposal. Used (gray): Already consumed.',
      faqQ12: 'How do I search for ingredients?',
      faqA12: 'Use the search bar at the top of the ingredients page to find ingredients by name, location, or category.',
      faqQ13: 'How do I filter ingredients by status?',
      faqA13: 'Use the status buttons (Fresh, Near Expiry, Expired, Used) at the top of the ingredients page to filter your view.',
      faqQ14: 'What is the freshness score?',
      faqA14: 'The freshness score is calculated based on the quality of your ingredients. Higher scores indicate better food management and less waste.',
      faqQ15: 'How do I import/export my data?',
      faqA15: 'Go to Settings > Data Management to export your data as CSV files or import data from previous backups.',
      faqQ16: 'How do I change the app language?',
      faqA16: 'Go to Settings > Language to select from available languages. The app supports multiple languages.',
      faqQ17: 'What is the weekly cleanup feature?',
      faqA17: 'The weekly cleanup automatically removes ingredients that have been marked as "used" for more than a week, helping keep your inventory clean.',
      faqQ18: 'How do I use the seasonal fruits feature?',
      faqA18: 'The app shows seasonal fruit recommendations based on the current season, helping you choose the freshest produce.',
      faqQ19: 'How do I rate the app?',
      faqA19: 'Go to Settings > About to rate the app in the app store. Your feedback helps us improve!',
      faqQ20: 'How do I clear all data?',
      faqA20: 'Go to Settings > Data Management > Clear All Data. This will permanently delete all your ingredients and cannot be undone.',
    ...
    forms: {
      ...
      expirationDateMustBeAfterPurchase: 'Expiration date must be after purchase date.',
      notes: 'Notes',
  ...
  zh: {
    ...
    statistics: {
      ...
      freshnessScoreCalculation: '新鲜度评分是如何计算的？',
      scoringWeights: '评分权重',
      freshItems: '新鲜物品',
      nearExpiryItems: '即将过期物品',
      expiredItems: '过期物品',
      usedItems: '已使用物品',
      calculationFormula: '计算公式',
      freshnessFormula: '新鲜度评分 = (新鲜 × 1.0 + 即将过期 × 0.5 + 过期 × 0.0 + 已使用 × 0.8) ÷ 总物品数 × 100%',
      example: '示例',
      freshnessExample: '如果你有8个新鲜物品、2个即将过期、1个过期、4个已使用物品（共15个），你的新鲜度评分为：(8×1.0 + 2×0.5 + 1×0.0 + 4×0.8) ÷ 15 × 100% = 13.2 ÷ 15 × 100% = 88%',
      defaultScoreRule: '默认分规则',
      defaultScoreExplanation: '当某周没有物品时，新鲜度评分为100%（满分），表示管理完美，没有浪费风险。',
      weeklyFreshnessTrend: '每周新鲜度趋势',
    ...
    settings: {
      ...
      about: '关于',
      version: '版本',
      versionStoreError: '无法打开应用商店。请手动在应用商店中搜索"Fridgy"。',
      legal: '法律条款',
      privacyPolicy: '隐私政策',
    ...
    support: {
      ...
      faqQ6: '统计页面的数据是如何计算的？',
      faqA6: '统计页面中的关键指标和状态概览是根据选择的时间段内新添加的食材来计算的。例如，当您选择"周"时，显示的是从本周一到今天新添加的食材统计数据。',
      faqQ7: '如何使用语音输入添加食材？',
      faqA7: '在食材添加页面，按住麦克风按钮录制您的食材详情。应用会使用AI分析并自动填写信息。',
      faqQ8: 'AI分析功能是什么？',
      faqA8: 'AI分析功能基于您的食材提供智能的存储位置建议和烹饪建议，还可以帮助制定用餐计划和减少浪费的技巧。',
      faqQ9: '如何设置通知？',
      faqA9: '前往设置 > 通知来启用每日提醒、即将过期提醒和过期物品提醒。您还可以设置自定义提醒时间。',
      faqQ10: '如何编辑或删除食材？',
      faqA10: '点击任何食材卡片进行编辑，或向左滑动食材显示删除选项。您也可以使用长按菜单获得更多选项。',
      faqQ11: '食材的不同状态是什么意思？',
      faqA11: '新鲜（绿色）：可以使用。即将过期（橙色）：尽快使用。已过期（红色）：需要处理。已使用（灰色）：已消耗。',
      faqQ12: '如何搜索食材？',
      faqA12: '使用食材页面顶部的搜索栏，可以按名称、位置或类别查找食材。',
      faqQ13: '如何按状态筛选食材？',
      faqA13: '使用食材页面顶部的状态按钮（新鲜、即将过期、已过期、已使用）来筛选您的视图。',
      faqQ14: '新鲜度评分是什么？',
      faqA14: '新鲜度评分是根据您的食材质量计算的。更高的分数表示更好的食物管理和更少的浪费。',
      faqQ15: '如何导入/导出我的数据？',
      faqA15: '前往设置 > 数据管理来将您的数据导出为CSV文件或从之前的备份导入数据。',
      faqQ16: '如何更改应用语言？',
      faqA16: '前往设置 > 语言来选择可用语言。应用支持多种语言。',
      faqQ17: '每周清理功能是什么？',
      faqA17: '每周清理会自动删除标记为"已使用"超过一周的食材，帮助保持您的库存清洁。',
      faqQ18: '如何使用季节性水果功能？',
      faqA18: '应用会根据当前季节显示季节性水果推荐，帮助您选择最新鲜的农产品。',
      faqQ19: '如何评价应用？',
      faqA19: '前往设置 > 关于来在应用商店中评价应用。您的反馈帮助我们改进！',
      faqQ20: '如何清除所有数据？',
      faqA20: '前往设置 > 数据管理 > 清除所有数据。这将永久删除您的所有食材且无法撤销。',
      contactUs: '联系我们',
      contactEmail: '邮件支持',
    ...
    forms: {
      ...
      expirationDateMustBeAfterPurchase: '过期日期必须晚于存放日期。',
      notes: '备注',
